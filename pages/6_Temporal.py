...
st.markdown('<p class="section-label"><span>Complaint Heatmap / Hour x Day of Week</span></p>', unsafe_allow_html=True)
...
fig_heatmap.update_layout(
    ...
    yaxis=dict(tickmode='array', tickvals=list(range(24)), ticktext=[f"{h}:00" for h in range(24)], tickfont={'family': 'Inter, sans-serif', 'size': 10}),
    xaxis=dict(tickfont={'family': 'Inter, sans-serif', 'size': 11}),
    coloraxis_colorbar=dict(title=dict(text="Complaints", font={'family': 'Inter, sans-serif', 'size': 11}), tickfont={'family': 'Inter, sans-serif', 'size': 10}),
...
with col_hour:
    st.markdown('<p class="section-label"><span>Complaints by Hour</span></p>', unsafe_allow_html=True)
    ...
    fig_hour.update_layout(height=350, showlegend=False, font_family="Inter, sans-serif", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', coloraxis_showscale=False, xaxis={'tickfont': {'size': 9}, 'gridcolor': '#e5e5e5'}, yaxis={'gridcolor': '#e5e5e5'}, margin={'l': 40, 'r': 20, 't': 20, 'b': 40})
    st.plotly_chart(fig_hour, use_container_width=True)

with col_day:
    st.markdown('<p class="section-label"><span>Complaints by Day of Week</span></p>', unsafe_allow_html=True)
    ...
    fig_day.update_layout(height=350, showlegend=False, font_family="Inter, sans-serif", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', coloraxis_showscale=False, xaxis={'categoryorder': 'array', 'categoryarray': day_names, 'tickfont': {'size': 10}, 'gridcolor': '#e5e5e5'}, yaxis={'gridcolor': '#e5e5e5'}, margin={'l': 40, 'r': 20, 't': 20, 'b': 40})
    st.plotly_chart(fig_day, use_container_width=True)

# Monthly trends
st.markdown('<p class="section-label"><span>Monthly Complaint Volume / Rat Season</span></p>', unsafe_allow_html=True)
...
fig_month.update_layout(height=350, showlegend=False, font_family="Inter, sans-serif", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', coloraxis_showscale=False, xaxis={'categoryorder': 'array', 'categoryarray': month_year_counts['Month'].tolist(), 'tickfont': {'size': 10}, 'gridcolor': '#e5e5e5'}, yaxis={'gridcolor': '#e5e5e5'}, margin={'l': 40, 'r': 20, 't': 20, 'b': 60})
st.plotly_chart(fig_month, use_container_width=True)

st.markdown(f'<div style="background:#fafafa;border:1px solid #e7e7e7;border-left:3px solid #0a0a0a;border-radius:6px;padding:1rem 1.25rem;font-size:0.9375rem;"><strong>Peak Rat Season:</strong> {peak_month_name} with {peak_complaints:,} complaints</div>', unsafe_allow_html=True)
